package com.example.quickindexbar.adapter;
...
import com.example.quickindexbar.R;
...
public class HaoHanAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		ViewHolder mViewHolder = ViewHolder.getHolder(view); //ViewHolder的另一种使用方式
		
		Person p = persons.get(position);
		
		String str = null;
		String currentLetter = p.pinyin.charAt(0) + "";
		// 根据上一个首字母,决定当前是否显示字母
		if(position == 0){
			str = currentLetter;
		}else {
			// 上一个人的拼音的首字母
			String preLetter = persons.get(position - 1).pinyin.charAt(0) + "";
		...
		mViewHolder.mName.setText(p.name);
